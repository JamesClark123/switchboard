...
func TestClearScreenCSI(t *testing.T) {
	s := New(20, 3)
	_, _ = s.Write([]byte("junk\r\nmore"))
	// ESC[2J erases screen; ESC[H homes cursor.
	_, _ = s.Write([]byte("\x1b[2J\x1b[Hclean"))
...
func TestSGRColoringSurvivesRender(t *testing.T) {
	s := New(20, 2)
	_, _ = s.Write([]byte("\x1b[31mRED\x1b[0m"))
...
func TestSaveRestoreCursorCSI(t *testing.T) {
	s := New(20, 5)
	_, _ = s.Write([]byte("\x1b[3;5H\x1b[s")) // save at (row=2,col=4)
	_, _ = s.Write([]byte("\x1b[1;1H"))       // move to origin
	_, _ = s.Write([]byte("\x1b[u"))          // restore
	col, row := s.Cursor()
	if col != 4 || row != 2 {
...
func TestSaveRestoreCursorESC(t *testing.T) {
	s := New(20, 5)
	_, _ = s.Write([]byte("\x1b[3;5H\x1b7")) // ESC 7 saves
	_, _ = s.Write([]byte("\x1b[1;1H"))
	_, _ = s.Write([]byte("\x1b8")) // ESC 8 restores
	col, row := s.Cursor()
	if col != 4 || row != 2 {
...
func TestReverseIndexInMiddle(t *testing.T) {
	s := New(10, 3)
	_, _ = s.Write([]byte("\x1b[2;1H")) // row 1
	_, _ = s.Write([]byte("\x1bM"))
	if _, row := s.Cursor(); row != 0 {
...
func TestIndexAndNextLineEsc(t *testing.T) {
	s := New(10, 3)
	_, _ = s.Write([]byte("\x1bD")) // IND — linefeed
	if _, row := s.Cursor(); row != 1 {
		t.Fatalf("IND row = %d, want 1", row)
	}
	_, _ = s.Write([]byte("\x1bE")) // NEL — next line, col 0
	c, r := s.Cursor()
	if c != 0 || r != 2 {
...
func TestEraseDisplayAboveAndBelow(t *testing.T) {
	s := New(10, 4)
	_, _ = s.Write([]byte("row0\r\nrow1\r\nrow2\r\nrow3"))
	// Move to row 2, erase from cursor downward (mode 0).
	_, _ = s.Write([]byte("\x1b[3;1H\x1b[0J"))
	stripped := stripStyle(s.Render())
	...
	s.Reset()
	_, _ = s.Write([]byte("row0\r\nrow1\r\nrow2\r\nrow3"))
	_, _ = s.Write([]byte("\x1b[3;1H\x1b[1J"))
	stripped = stripStyle(s.Render())
	if strings.Contains(stripped, "row0") || strings.Contains(stripped, "row1") {
...
func TestEraseLineFromBOLToCursor(t *testing.T) {
	...
	_, _ = s.Write([]byte("\x1b[1;6H\x1b[1K"))
	stripped := stripStyle(s.Render())
...
func TestEraseLineEntire(t *testing.T) {
	s := New(20, 2)
	_, _ = s.Write([]byte("keepit"))
	_, _ = s.Write([]byte("\x1b[2K"))
	if got := firstNonEmptyLine(s.Render()); got != "" {
...
func TestInsertAndDeleteLines(t *testing.T) {
	...
	_, _ = s.Write([]byte("\x1b[2;1H\x1b[1M"))
	stripped := stripStyle(s.Render())
	...
	_, _ = s.Write([]byte("\x1b[2;1H\x1b[1L"))
	stripped = stripStyle(s.Render())
	lines = strings.Split(stripped, "\n")
...
func TestUnknownCSIIgnored(t *testing.T) {
	...
	s := New(10, 2)
	_, _ = s.Write([]byte("\x1b[5;20r")) // DECSTBM ignored
	_, _ = s.Write([]byte("\x1b[?25h"))  // DECSET cursor visible — ignored
	_, _ = s.Write([]byte("\x1b[?25l"))  // DECRST
	_, _ = s.Write([]byte("\x1b[10t"))   // window manipulation
	_, _ = s.Write([]byte("ok"))
...
func TestScrollBoundaryEdgeCases(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, _ = s.Write([]byte("\x1b[1;1H\x1bM"))
	}
...
func TestClampNegativeArguments(t *testing.T) {
	...
	_, _ = s.Write([]byte("\x1b[1;1H\x1b[10A\x1b[10D"))
	col, row = s.Cursor()
	if col != 0 || row != 0 {
...
func TestCursorHiddenByDECTCEM(t *testing.T) {
	s := New(10, 2)
	_, _ = s.Write([]byte("hi"))
	_, _ = s.Write([]byte("\x1b[?25l")) // hide cursor
	if hasReverse(s.Render()) {
		t.Fatalf("cursor should be hidden after ?25l: %q", s.Render())
	}
	_, _ = s.Write([]byte("\x1b[?25h")) // show cursor again
	if !hasReverse(s.Render()) {
...
func TestCursorOverlayDoesNotCorruptContent(t *testing.T) {
	...
	s := New(10, 2)
	_, _ = s.Write([]byte("\x1b[1;1Hab")) // cursor ends at col 2
	_, _ = s.Write([]byte("\x1b[1;1H"))   // move cursor back onto 'a'
