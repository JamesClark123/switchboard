...
func TestAddKitRecordsSourceAndRestores(t *testing.T) {
	...
	out, err := m.AddKit(ctx, sb.GetId(), "/kits/ruff", nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAddKitEmitsNonRunningSoTerminalIsTornDown(t *testing.T) {
	...
	if _, err := m.AddKit(ctx, sb.GetId(), "/kits/ruff", nil, nil, nil); err != nil {
		t.Fatal(err)
	}
...
func TestAddKitRequiresSource(t *testing.T) {
	m, _, _, dir := newTestManager(t)
	sb, _ := launchOne(t, m, dir, "proj")
	if _, err := m.AddKit(context.Background(), sb.GetId(), "  ", nil, nil, nil); err == nil {
