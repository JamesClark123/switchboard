...
# allowlist and runs the fixed, pre-authorized string on the host. Async: this
# returns immediately; the result is delivered back to the agent as a follow-up
# message when the run finishes.
name="$1"
curl -s -X POST -H 'Content-Type: application/json' \
  -d "{\"sandbox_id\":\"<embedded-id>\",\"name\":\"$name\"}" \
  http://host.docker.internal:<port>/escape-hatch/run
```

- It sends **only** `{sandbox_id, name}` — never a command string. There is no argument by which the
...
```json
{ "sandbox_id": "<uuid>", "name": "install-deps" }
```
